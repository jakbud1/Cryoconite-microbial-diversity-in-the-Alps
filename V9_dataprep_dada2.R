...
out_V9_r1 <- filterAndTrim(files_V9_r1, filts_V9_r1,  minLen = 85, maxLen = 160,
                            maxN = 0, maxEE = 0.5, truncQ = 2, rm.phix = FALSE,
                            compress = TRUE, trimLeft = 15, trimRight = 15); head(out_V9_r1, 50)
 
## Train parametric error model  
err_V9_r1 <- learnErrors(filts_V9_r1, randomize = TRUE)
 
png("Output_dada2/V9_r1_error_model.png", width = 800, height = 800)
plotErrors(err_V9_r1, nominalQ = TRUE)
dev.off()
 
## Core sample inference algorithm
dadas_V9_r1 <- dada(filts_V9_r1, err = err_V9_r1)
dadas_V9_r1[[1]]
...
out_V9_r2 <- filterAndTrim(files_V9_r2, filts_V9_r2,  minLen = 85, maxLen = 160,
                            maxN = 0, maxEE = 0.5, truncQ = 2, rm.phix = FALSE,
                           compress = TRUE, trimLeft = 15, trimRight = 15); head(out_V9_r2, 50)
 
## Train parametric error model  
err_V9_r2 <- learnErrors(filts_V9_r2, randomize = TRUE)
 
png("Output_dada2/V9_r2_error_model.png", width = 800, height = 800)
plotErrors(err_V9_r2, nominalQ = TRUE)
dev.off()
 
## Core sample inference algorithm
dadas_V9_r2 <- dada(filts_V9_r2, err = err_V9_r2)
dadas_V9_r2[[1]]
...
taxa_V9_all <- assignTaxonomy(seqtab.nochim_V9_r2, "Input/Seq/V9/silva_128.18s.99_rep_set.dada2.fa.gz",
                              tryRC = TRUE)
...
taxa1.silva <- setDT(as.data.frame(taxa_V9_all), keep.rownames = TRUE)
colnames(taxa1.silva)[colnames(taxa1.silva) == c("rn")] <- c("ASV_seq")
taxa.abundance.silva <- merge(seq.abundancesV9_all, taxa1.silva, by = "ASV_seq")
write.csv(taxa.abundance.silva, "Output_dada2/taxa.silva_V9_all.csv", row.names = FALSE)
...
write.csv(track_V9_all, file = "Output_dada2/track_reads_V9.csv", row.names = FALSE)
